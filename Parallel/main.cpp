...
//Fills in a random matrix recursively
void populateMatrixZOrder(int *M, int n){
	if (n==1){
		M[0] = rand()%(MAX_NUM - MIN_NUM + 1) + MIN_NUM;
	}else{
		cilk_spawn populateMatrixZOrder(&M[0],n/2); //Fills in Section 1,1
		cilk_spawn populateMatrixZOrder(&M[n*n/4],n/2);//Fills in section 1,2
		cilk_spawn populateMatrixZOrder(&M[n*n/2],n/2);//Fills in Section 2,1
		cilk_spawn populateMatrixZOrder(&M[n*n/2 + n*n/4],n/2);//Fills in Section 2,2
		cilk_sync;
	}
};

int* initializeMatrix(int n){	
	int* matrix = new int[n*n];	//Array of length n*n to hold Zorder
...
void Mult(int *C, int *A, int *B, int n){
	...
	if (n ==1){//BASE CASE ONCE DOWN TO UNIT MATRICES
		C[0] = C[0] + A[0] * B[0]; //Given in slides
	}else{
		...
		cilk_spawn Mult(&C[0],&A[0],&B[0], n/2);
		cilk_spawn Mult(&C[n*n/4],&A[0],&B[n*n/4],n/2);
		cilk_spawn Mult(&C[n*n/2 + n*n/4],&A[n*n/2],&B[n*n/4],n/2);
		cilk_spawn Mult(&C[n*n/2],&A[n*n/2],&B[0],n/2);
		cilk_sync;
		cilk_spawn Mult(&C[n*n/2],&A[n*n/2 + n*n/4],&B[n*n/2], n/2);
		cilk_spawn Mult(&C[n*n/2 + n*n/4],&A[n*n/2 + n*n/4],&B[n*n/2 + n*n/4],n/2);
		cilk_spawn Mult(&C[n*n/4],&A[n*n/4],&B[n*n/2 + n*n/4],n/2);
		cilk_spawn Mult(&C[0],&A[n*n/4],&B[n*n/2],n/2);
		cilk_sync;
	}	
};
//Print Out given array, in the case of this program Zorder
...
//Prints a Zordered matrix out in Rowwise Order
